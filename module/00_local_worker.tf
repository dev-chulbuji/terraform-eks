locals {
  ...
  workers_group_defaults_defaults = {
    ...
    subnets                       = var.subnet_ids              # A list of subnets to place the worker nodes in. i.e. ["subnet-123", "subnet-456", "subnet-789"]
  ...
  worker_roles = [
    for k, v in local.node_groups_expanded : {
      worker_role_arn = lookup(v, "iam_role_arn", aws_iam_role.workers.arn)
    }
  ]
}
